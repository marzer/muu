...
// https://github.com/google/mathfu/blob/master/unit_tests/quaternion_test/quaternion_test.cpp
///////////////////////////////////////////////////////////////////////////////////
...
TEMPLATE_BATCHED_TEST("quaternion constructors", all_quaternions)
{
	...
	{
		const auto q = quat_t{};
		quat_for_each(q, [](auto s, size_t) { CHECK(s == T{}); });
	}
	...
	BATCHED_SECTION("scalar + vector constructor")
	{
...
TEMPLATE_BATCHED_TEST("quaternion dot", all_quaternions)
{
	...
	quat_for_each(q1,
				  q2,
				  [&](auto s1, auto s2, size_t) noexcept
				  {
					  MUU_FMA_BLOCK;
...
TEMPLATE_BATCHED_TEST("quaternion euler", all_quaternions)
{
	...
	for (const auto& [input_dir, rot, expected_dir] : values)
	{
		...
		{
			TEST_INFO("   input direction: " << input_dir);
			TEST_INFO("    input rotation: " << rot);
			TEST_INFO("        quaternion: " << quat);
			TEST_INFO("  output direction: " << dir);
			TEST_INFO("expected direction: " << expected_dir);
		...
		if (!approx_equal(dir2, expected_dir, static_cast<T>(s::default_epsilon * 4)))
		{
			TEST_INFO("    input direction: " << input_dir);
			TEST_INFO("     input rotation: " << rot);
			TEST_INFO("output quaternion 1: " << quat);
			TEST_INFO("  output rotation 1: " << rot2);
			TEST_INFO("output quaternion 2: " << quat2);
			TEST_INFO("   output direction: " << dir2);
			TEST_INFO(" expected direction: " << expected_dir);
...
TEMPLATE_BATCHED_TEST("quaternion conjugate", all_quaternions)
{
	...
	quat_t q{ static_cast<T>(1.4), static_cast<T>(6.3), static_cast<T>(8.5), static_cast<T>(5.9) };
	q.normalize();
...
{
	template <typename T>
	static void quat_slerp_test_case(intmax_t angle, T alpha, intmax_t expected_angle) noexcept
	{
		// Checks equality of
...
TEMPLATE_BATCHED_TEST("quaternion multiplication", all_quaternions)
{
	...
	const auto axis = normalize(vec3_t{ static_cast<T>(4.3), static_cast<T>(7.6), static_cast<T>(1.2) });
