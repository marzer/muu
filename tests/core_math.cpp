...
// functions internally at runtime for float128_ts, otherwise they downcast to long double and upcast the result.
...
namespace
{
	...
	template <typename T>
	using std_func_ptr = T(*)(T) noexcept;

	template <typename T>
	void math_test_runtime(muu_func_ptr<T> muu_func, std_func_ptr<T> std_func, T start, T end, size_t subdivs) noexcept
	{
	...
	template <typename T, muu_func_ptr<T> Func, typename Start, typename End, size_t Subdivs>
	[[maybe_unused]]
	inline constexpr std::array<table_value<T>, Subdivs + 1u> lookup_table
		= []() constexpr noexcept
		{
			std::array<table_value<T>, Subdivs + 1u> table{};  // +1 for inclusive end
	...
	{
		static constexpr auto& table = lookup_table<T, Func, Start, End, Subdivs>;
...
#define MATH_CHECK_TYPE(type, func, subdivs)												\
	{																						\
		...
		math_test_runtime<T>(muu::func, MUU_CONCAT(std_, func)<T>, start::value, end::value, subdivs);	\
...
#define MATH_STD_FUNC_SHIM(func)															\
	template <typename T>																	\
	static T MUU_CONCAT(std_, func)(T val) noexcept											\
	{																						\
...
#define MATH_CHECKS(func, start_val, end_val, subdivs)							\
	...
	TEST_CASE("core - " MUU_MAKE_STRING(func))									\
	{																			\
