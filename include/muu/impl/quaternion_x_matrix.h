...
#undef MAT_GET
	...
	MUU_PURE_GETTER
	inline constexpr quaternion<Scalar> quaternion<Scalar>::from_rotation(
		MUU_VC_PARAM(matrix<Scalar, 3, 3>) rot) noexcept
	{
	...
	MUU_PURE_GETTER
	inline constexpr quaternion<Scalar> quaternion<Scalar>::from_rotation(
		MUU_VC_PARAM(matrix<Scalar, 3, 4>) rot) noexcept
	{
		if constexpr (is_small_float)
			return quaternion{ promoted_quat::from_rotation(matrix<promoted_scalar, 3, 3>{ rot }) };
		else
	...
	MUU_PURE_GETTER
	inline constexpr quaternion<Scalar> quaternion<Scalar>::from_rotation(
		MUU_VC_PARAM(matrix<Scalar, 4, 4>) rot) noexcept
	{
		if constexpr (is_small_float)
			return quaternion{ promoted_quat::from_rotation(matrix<promoted_scalar, 3, 3>{ rot }) };
		else
