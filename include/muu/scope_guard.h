...
namespace muu
{
	...
	namespace impl
	{
		...
		template <typename T, scope_guard_mode Mode,
			bool ExceptionAware = (Mode < scope_guard_mode::invoke_always)
		>
		struct scope_guard_storage
		{
			...
			template <typename U>
			scope_guard_storage(U&& callable) noexcept(std::is_reference_v<T> || std::is_nothrow_constructible_v<T, U&&>)
		...
		template <typename T, scope_guard_mode Mode,
			bool Movable = (std::is_reference_v<T> || (std::is_move_constructible_v<T> && std::is_move_assignable_v<T>))
		>
		...
		class scope_guard_ : protected scope_guard_move<T, Mode>
		{
			...
			static_assert(
				std::is_invocable_v<std::add_lvalue_reference_t<std::remove_reference_t<T>>>,
	...
	public:
		...
		MUU_NODISCARD_CTOR
		explicit
		scope_guard(U&& func) noexcept(std::is_nothrow_constructible_v<base, U&&>)
	...
	public:
		...
		MUU_NODISCARD_CTOR
		explicit
		scope_fail(U&& func) noexcept(std::is_nothrow_constructible_v<base, U&&>)
			: base{ static_cast<U&&>(func) }
		{
		}
	...
	public:
		...
		MUU_NODISCARD_CTOR
		explicit
		scope_success(U&& func) noexcept(std::is_nothrow_constructible_v<base, U&&>)
			: base{ static_cast<U&&>(func) }
		{
		}
